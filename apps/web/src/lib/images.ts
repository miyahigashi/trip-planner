...
export async function saveImageFromUrl(srcUrl: string): Promise<SavedImage> {
  // ランタイム安全策（Edge 実行を避ける）
  if ((process as any).env?.NEXT_RUNTIME === "edge") {
    throw new Error("saveImageFromUrl must run on Node.js runtime");
  }
  ...
  if (!e800) await w800.save(webp800, { resumable: false, contentType: "image/webp", metadata: { cacheControl: "public, max-age=31536000, immutable" } });
  if (!e400) await w400.save(webp400, { resumable: false, contentType: "image/webp", metadata: { cacheControl: "public, max-age=31536000, immutable" } });
