...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      infoRef.current = new google.maps.InfoWindow({ maxWidth: 280 });
        ...
        m.addListener("click", async () => {
          ...
          const html = `
            <div style="max-width:100%;display:block;">
              <div style="font-weight:600;margin-bottom:6px;font-size:14px;line-height:1.3">${escapeHtml(
                it.name
              )}</div>
              ${
                img
              ...
              ${
                it.address
                  ? `<div style="font-size:12px;color:#555;line-height:1.4">${escapeHtml(
                      it.address
                    )}</div>`
                  : ""
              }
              <div style="margin-top:8px;">
                <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                  it.name
                )}&query_place_id=${encodeURIComponent(it.placeId)}" target="_blank" rel="noreferrer"
  ...
  const zoomToCurrentLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        myLocRef.current.marker = marker;
  ...
  return (
    // svh/dvh フォールバック: まず svh、未対応なら dvh
